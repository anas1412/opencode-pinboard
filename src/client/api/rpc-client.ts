...
const REST_MAP: Record<string, RestRoute> = {
  ...
  createAskChat: { method: "POST", path: "/api/ask" },
  listAskChats: { method: "GET", path: "/api/ask" },
  renameAskChat: { method: "PATCH", path: (p) => `/api/ask/${p.id}/rename` },
  deleteAskChat: { method: "DELETE", path: (p) => `/api/ask/${p.id}` },
