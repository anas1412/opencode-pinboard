...
REPO="anas1412/opentrack"
BRANCH="main"
INSTALL_DIR="${OPENTRACK_DIR:-$HOME/opentrack}"
DATA_DIR="${OPENTRACK_DATA_DIR:-$HOME/.opentrack}"
...
check_deps() {
  ...
  if [ "$missing" = true ]; then
    ...
    echo "    curl -fsSL https://raw.githubusercontent.com/$REPO/$BRANCH/opentrack.sh | bash"
    exit 1
  fi
}
...
cmd_update() {
  ...
  if [ ! -d "$INSTALL_DIR/.git" ]; then
    ...
    echo "  Install it first: curl -fsSL https://raw.githubusercontent.com/$REPO/$BRANCH/opentrack.sh | bash"
    exit 1
  fi
