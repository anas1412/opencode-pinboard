# OpenTack — Product Requirements Document
...
OpenTack is a local browser-based workspace where every unit of work is a **ticket** and every ticket spawns a **live opencode session** directly in the browser. The developer opens a ticket, a terminal panel appears running opencode already scoped to the correct repo and branch, the agent works, the developer reviews the diff, and closes the ticket. No new terminal window, no context switching, no Docker.

The entire loop — task definition, agent execution, diff review, and commit — happens in one browser tab at `localhost:3000`.

### What makes it different

OpenTack treats the **ticket as the primitive**. Every opencode session is permanently linked to the ticket that spawned it: the task description, session transcript, token cost, files changed, and git diff are all stored together. The developer always knows what was asked, what the agent did, what it cost, and exactly what changed.

### Scope of this document

This document covers **OpenTack Free** — a single-developer, local-only product. No accounts, no cloud, no teams. It runs as a local server (`bunx opentack` or an installable binary) and is accessed via the browser.
...
| Setup steps for a new developer | `bunx opentack` → browser opens → done |
...
OpenTack Server (Bun + Fastify)        localhost:3000
    │
    ├── REST API          Drizzle ORM → SQLite (~/.opentack/db.sqlite)
...
OpenTack runs as a local Bun server. node-pty spawns opencode as a child process with a PTY, streams its output over WebSocket to xterm.js in the browser, and captures everything for the transcript simultaneously. This is a single Bun process — no separate binary to install or manage. The developer installs OpenTack, and that's the only install they do. opencode itself is managed by OpenTack (see §7).
...
**SQLite** at `~/.opentack/db.sqlite`. Managed by Drizzle ORM with Drizzle Kit for migrations. Holds all structured operational data: tickets, sessions, cost records, repo metadata. Transcripts stored as JSON text. A single file the developer can back up, inspect, or delete. No external database process.

**Filesystem** — project-specific content lives as `.md` files in each repo's `.opentack/` directory (see §5.5). Global templates live in `~/.opentack/templates/` (see §6.4).

### 4.5 Distribution

Shipped as an npm package: `bun install -g opentack` or `bunx opentack`. Running `opentack` starts the server and opens `localhost:3000` in the default browser. All state lives in `~/.opentack/`.

### 4.6 Logging

Structured JSON logs written to `~/.opentack/logs/opentack-{YYYY-MM-DD}.log`. One file per day, never deleted by OpenTack — the developer manages disk space.
...
Default level: `info`. Configurable via `OPENTACK_LOG_LEVEL` env var.
...
xterm.js natively sanitizes terminal escape sequences — OpenTack relies on this.
...
interface Repo {
  id:             string
  name:           string
  localPath:      string          // absolute path on disk, .opentack/ derived from here
...
Project-specific content lives inside each registered repo's `.opentack/` directory. These files are in the repo — versioned, cloned, committed alongside code.
...
~/projects/my-api/                     # registered repo
  ...
  └── .opentack/
      ...
      └── notes.md                     ← developer scratch notes, OpenTack ignores it
```

| File | Purpose | Read by OpenTack? |
|------|---------|------------------|
...
OpenTack never writes to these files. They are authoring surfaces for the developer, edited in their own editor. If a file doesn't exist, OpenTack proceeds normally.

**Global templates** live in `~/.opentack/templates/` (see §6.4).
...
- **Category** — segmented control: Feature · Bug · Refactor · Chore · Docs. If a matching template exists at `~/.opentack/templates/{category}.md`, it pre-fills the description. The developer can edit freely before saving.
...
OpenTack does not manage opencode's config, providers, or auth — opencode handles all of that itself via `~/.config/opencode/` and `opencode auth`. The developer sets up opencode once on their machine and OpenTack just runs it.
...
OpenTack always wants to run the latest version of opencode. On every server startup:
...
When a session starts, OpenTack checks for `{repo.localPath}/.opentack/context.md`. If the file exists, it's prepended to the session prompt before the ticket description:
...
2. Accepted changes are committed: `git add -A && git commit -m "opentack: {ticket.title} [#{ticket.id}]"`
...
Browser tab title shows the active session cost when a session is running: `OpenTack · $0.14`.
...
- `opentack` CLI: starts server, opens browser
...
**Done when:** A developer uses OpenTack as their daily driver without friction.
...
| Concern | Choice |
|---------|--------|
| Package | npm (`opentack` / `opentack-ai`) |
| Install | `bun install -g opentack` or `bunx opentack` (also works via `npm install -g opentack`) |
| Data dir | `~/.opentack/` |
| DB file | `~/.opentack/db.sqlite` |
| Logs | `~/.opentack/logs/` |
| Templates | `~/.opentack/templates/` — global per-category ticket templates (`bug.md`, `feature.md`, etc.) |
...
npm (`opentack` / `opentack-ai`). Published via `bun publish` (compatible with npm registry).
...
`package.json` `bin` field: `{"opentack": "./dist/server/cli.js"}`

### Vercel — project site

The GitHub repo is linked to Vercel for automatic deployment of:
- **Landing page / marketing site** at `opentack.dev` (or similar domain)
- **Documentation site** at `docs.opentack.dev` (Phase 2)

Vercel auto-deploys on every push to `main`. No manual deploy steps. The OpenTack application itself is not hosted on Vercel — it's a local tool.
